    ...
    public static class KpopChapterSelect
    {
        ...
        public static void Open(GameManager gm, Action<int> onPick, Action<int> onPlay)
        {
            ...
            for (int i = 0; i < Timeline.Chapters; i++)
            {
                ...
                // 56차-2: 옆 칸 글자와 붙지 않게 — 상자를 카드 폭 안으로, 글자는 상자에 맞춰 줄어든다
                var st = CoastHudLayout.MakeText(root, "St" + n, stateTxt, 11, TextAnchor.MiddleCenter,
                    new Vector2(0f, 1f), new Vector2(0f, 1f), new Vector2(x0 + col * (cw + gap), -rowY[row] - ch - 22f), new Vector2(x0 + col * (cw + gap) + cw, -rowY[row] - ch - 2f));
                st.resizeTextForBestFit = true; st.resizeTextMinSize = 7; st.resizeTextMaxSize = CoastHudLayout.Scaled(11);
